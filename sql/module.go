...
// QueryCount avoids allocating memory for the result set and outputs only the count of rows
func (dbase *Database) QueryCount(query string, args ...interface{}) (int, error) {
	return dbase.QueryCountWithContext(dbase.ctx(), query, args...)
}

func (dbase *Database) QueryCountWithContext(ctx context.Context, query string, args ...interface{}) (int, error) {
	rows, err := dbase.db.QueryContext(ctx, query, args...)
	if err != nil {
		return 0, err
	}
	...
	for rows.Next() {
		count++
		// Don't scan any data, just count the rows
	}
...
// ForceGc is a hook to manually force gc. Not clear if k6 exposes it somewhere [debug]
func (dbase *Database) ForceGc() {
	runtime.GC()
}
